...
#ifndef DAHUA_VECTOR_STRING_STORAGE_INC_
#define DAHUA_VECTOR_STRING_STORAGE_INC_
...
public: // protected:
    ...
    void append(ForwardIterator b, ForwardIterator e)
    {
        ...
        typedef typename std::iterator_traits<ForwardIterator>::distance_type diff_t;
#else
        typedef typename std::iterator_traits<ForwardIterator>::difference_type diff_t;
#endif
        const diff_t sz = std::distance(b, e);
        assert(sz >= 0);
        if (sz == 0) return;
...
#endif // DAHUA_VECTOR_STRING_STORAGE_INC_
