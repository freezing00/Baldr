...
/// \brief 通用相机对象
/// \~english
/// \brief universal camera object
typedef struct GENICAM_Camera
{
	/// \~chinese
	/// \brief 底层私有数据
	/// \~english
	/// \brief underlying private data
	void *priv;
    ...
    /// \retval 非零     失败
    /// \~english 
	/// \brief save the configuration of the camera. Overwrite the file if exists.
    /// \param [in] thiz this pointer
	...
	/// \return 返回用户实际写入的字节数
	/// \~english 
	/// \brief write serial data to camera serial register, will erase the data writen before (Only our own camera with Serial Port has this function)
	...
	/// \~english 
	/// \brief read serial data from camera serial register (Only our own camera with Serial Port has this function)
...
{
    GENICAM_ECameraAccessPermission eAccessStatus;			 ///< \~chinese Gige设备权限状态					\~english Gige device access status	
    uint32_t nReserved[7];									 ///< \~chinese 保留								\~english reserved field

    char strMACAddr[MAX_STRING_LENTH];						///< \~chinese  网卡Mac地址						    \~english Network card MAC Address
    char strIPAddr[MAX_STRING_LENTH];						///< \~chinese  设备Ip地址						    \~english Device ip Address
    char strSubnetMask[MAX_STRING_LENTH];					///< \~chinese  子网掩码							\~english SubnetMask
    char strDefaultGateWay[MAX_STRING_LENTH];				///< \~chinese  默认网关							\~english Default GateWay
    char strProtocolVersion[MAX_STRING_LENTH];				///< \~chinese  网络协议版本						\~english Net protocol version
    char strIPConfiguration[MAX_STRING_LENTH];				///< \~chinese  Ip配置有效性						\~english IP configuration valid
...
{
    int32_t bLowSpeedSupported;										///< \~chinese 1支持，0 不支持，其他值 非法。	\~english true support,false not supported,other invalid
    int32_t bFullSpeedSupported;									///< \~chinese 1支持，0 不支持，其他值 非法。	\~english true support,false not supported,other invalid
    int32_t bHighSpeedSupported;									///< \~chinese 1支持，0 不支持，其他值 非法。	\~english true support,false not supported,other invalid
    int32_t bSuperSpeedSupported;									///< \~chinese 1支持，0 不支持，其他值 非法。	\~english true support,false not supported,other invalid
    int32_t bDriverInstalled;										///< \~chinese 1安装，0 未安装，其他值 非法。	\~english true support,false not supported,other invalid
    uint32_t nReserved[3];											///< \~chinese 保留								\~english reserved field

    char strConfigurationValid[MAX_STRING_LENTH];					///< \~chinese 配置有效性						\~english Configuration Valid
    char strGenCPVersion[MAX_STRING_LENTH];							///< \~chinese GenCP 版本						\~english GenCP Version
    char strU3VVersion[MAX_STRING_LENTH];							///< \~chinese U3V 版本号						\~english U3v Version
    char strDeviceGUID[MAX_STRING_LENTH];							///< \~chinese 设备引导号						\~english Device guid number                  
    char strFamilyName[MAX_STRING_LENTH];							///< \~chinese 设备系列号						\~english Device serial number 
    char strU3VSerialNumber[MAX_STRING_LENTH];						///< \~chinese 设备序列号						\~english Device SerialNumber
    char strSpeed[MAX_STRING_LENTH];								///< \~chinese 设备传输速度						\~english Device transmission speed
    char strMaxPower[MAX_STRING_LENTH];								///< \~chinese 设备最大供电量					\~english Maximum power supply of device
    char strReserved[MAX_STRING_LENTH * 8];							///< \~chinese 保留								\~english reserved field
...
{
    int32_t nType;										 ///< \~chinese 设备类别			\~english camera type
    uint32_t nReserved[7];                               ///< \~chinese 保留				\~english reserved field

	char strKey[MAX_STRING_LENTH];                       ///< \~chinese 厂商:序列号			\~english camera key
    char strUserDefinedName[MAX_STRING_LENTH];			 ///< \~chinese 用户自定义名		\~english UserDefinedName
    char strSerialNumber[MAX_STRING_LENTH];				 ///< \~chinese 设备序列号			\~english Device SerialNumber
    char strVendor[MAX_STRING_LENTH];					 ///< \~chinese 厂商				\~english Camera Vendor
    char strModel[MAX_STRING_LENTH];					 ///< \~chinese 设备型号			\~english Device model
    char strManufactureInfo[MAX_STRING_LENTH];			 ///< \~chinese 设备制造信息		\~english Device ManufactureInfo
    char strDeviceVersion[MAX_STRING_LENTH];			 ///< \~chinese 设备版本			\~english Device Version
    char strReserved[MAX_STRING_LENTH * 9];				 ///< \~chinese 保留				\~english reserved field

    union
    {
        GigEDeviceInfo stGigEInfo;						///< \~chinese  Gige设备信息		\~english Gige Device Information
        UsbDeviceInfo stUsbInfo;						///< \~chinese  Usb设备信息			\~english Usb  Device Information
    }DeviceSpecificInfo;
...
/// \brief 全局单例系统对象
/// \~english
/// \brief Global singleton system object
...
typedef struct GENICAM_Frame
{
	...
	/// \return none
	/// \~english
	/// \brief Call this interface to release image related memory externally when it is not used
	/// \param [in] thiz this pointer
	/// \return none
	...
	/// \~english
	/// \brief Get the memory start address of this image data
	/// \param [in] thiz this pointer
	/// \return Return the memory start address of this image data  	
	...
	/// \brief 获取当前帧数据净荷类型
	/// \param [in] thiz 本接口指针
	/// \param [in/out] pPayloadTypeList 当前帧数据净荷类型种类集，最大种类不超过MAX_PAYLOAD_TYPE_CNT，该内存外部分配
	...
	/// \param [in] thiz this pointer
	/// \param [in/out] pPayloadTypeList Current frame data type set, maximum type cannot exceed MAX_PAYLOAD_TYPE_CNT, and the memory is allocated externally
	/// \param [in/out] pTypeCnt the actual length of array when input;the number of the current frame's data types when output, maximum value cannot exceed 20
	/// \retval < 0 get failed
...
/// \~chinese
/// \brief PCIE设备统计流信息结构体
/// \~english
/// \brief PCIE device stream statistics information structure
...
/// \brief U3V设备统计流信息结构体
/// \~english
/// \brief U3V device stream statistics information structure
typedef struct GENICAM_U3VStreamStatsInfo
{
	...
	//正常帧统计
	uint32_t    imageReceived;			///< \~chinese 正常获取的帧数					  \~english  Number of images error frames
	double      fps;                    ///< \~chinese 帧率								  \~english  Frame rate
	double      bandwidth;              ///< \~chinese 带宽(Mbps)						  \~english  Bandwidth(Mbps)
	uint32_t    reserved[8];            ///< \~chinese 预留							      \~english  Reserved field
...
/// \~chinese
/// \brief Gige设备统计流信息结构体
/// \~english
/// \brief Gige device stream statistics information structure
typedef struct GENICAM_GigEStreamStatsInfo
{
	...
	//帧相关统计
	uint32_t    imageError;				///< \~chinese 图像错误的帧数					\~english  Number of image error frames
	uint32_t    lostPacketBlock;		///< \~chinese 丢包的帧数						\~english  Number of frames lost
...
typedef struct GENICAM_StreamStatisticsInfo
{
	...
	union
	{
		GENICAM_PCIEStreamStatsInfo pcieStatisticsInfo;						///< \~chinese PCIE统计信息对象						 \~english  PCIE Statistics Object
		GENICAM_U3VStreamStatsInfo u3vStatisticsInfo;						///< \~chinese U3V统计信息对象						 \~english  U3V Statistics Object
		GENICAM_GigEStreamStatsInfo gigeStatisticsInfo;						///< \~chinese Gige统计信息对象						 \~english  GIGE Statistics Object
	};
...
typedef struct GENICAM_StreamSource
{
	...
	/// \param [in]  The time out value of getting one image, unit is MS; When the value is INFINITE which means infinite wait
	/// \return success:0, fail:-1
	...
	/// \~english
	/// \brief Register data frame callback function; This asynchronous frame acquisition mechanism and synchronous frame acquisition mechanism are mutually exclusive, only one method can be chosen between these two in system
	/// \brief only process one callback unction 
	/// \param [in] thiz this pointer
	/// \param [in] proc Data frame callback function; It is advised to not put time-consuming operation in this function, otherwise it will block follow-up data frames and affect real time performance
	/// \return success:0, fail:-1
	...
	/// \return 返回注册是否成功，0表示成功，-1表示失败
	/// \~english
	/// \brief Register data frame callback function(include user defined data); This asynchronous frame acquisition mechanism and synchronous frame acquisition mechanism are mutually exclusive, only one method can be chosen between these two in system
	/// \brief only process one callback unction 
	/// \param [in] thiz this pointer
	/// \param [in] proc Data frame callback function; It is advised to not put time-consuming operation in this function, otherwise it will block follow-up data frames and affect real time performance
    ...
    /// \~english
    /// \brief	set packet time out(ms),just for GigE devices
    /// \param [in] thiz this pointer
    /// \param [in] the value of time out
    /// \return success:0, fail:-1
	...
	/// \return 设置是否成功,0表示成功，-1表示失败
	/// \~english
	/// \brief get stream statistics information
	/// \param [in] the value of stream statistics information,
	/// \return success:0, fail:-1
	...
	/// \return 设置是否成功,0表示成功，-1表示失败
	/// \~english
	/// \brief reset stream statistics information
	/// \return success:0, fail:-1
...
/// \~chinese
/// \brief 创建流通道对象，该对象由外部调用该接口创建，释放时调用release接口
///
...
/// \retval 0   创建成功
/// \~english
/// \brief create stream channel object, this object is created by external call, and it requires to call release interface when release it
...
/// \brief 参数更新事件参数封装
/// \~english
/// \brief updating parameters event encapsulation
...
/// \brief 枚举：流事件状态
/// \~english
/// \brief enumeration:stream event status
...
/// \brief消息通道事件ID列表
/// \~english
/// \brief message channel event id list
...
typedef struct GENICAM_SMsgChannelArg
{
	...
	char paramNames[MAX_PARAM_CNT][MAX_STRING_LENTH];   ///< \~chinese 事件相关的属性名列表。约定属性名不超过1000个，每个属性名的字符串长度不超过256  \~english array of parameter's name which is related. the maximum number of parameters is 1000. and the maximum length of strings of each parameter is 255.
...
/// pConnectArg 回调时主动推送的连接事件对象
/// \~english
/// \brief call back function declaration of camera connection status event
/// pConnectArg The connection event which will be active pushed out during the callback
...
/// \~chinese
/// \brief 参数更新事件回调函数声明
/// pParamUpdateArg 回调时主动推送的参数更新事件对象
/// \~english
/// \brief call back function declaration of camera parameter update event
/// pParamUpdateArg The parameter update event which will be active pushed out during the callback
...
/// \~chinese
/// \brief 设备流事件回调函数声明
/// pStreamArg 回调时主动推送的流事件对象
/// \~english
/// \brief call back function declaration of stream event
/// pStreamArg The stream event which will be active pushed out during the callback
...
/// \~chinese
/// \brief 消息通道事件回调函数声明
/// pMsgChannelArg 回调时主动推送的消息通道事件对象
/// pUser  用户自定义数据
/// \~english
/// \brief call back function declaration of message channel event 
/// pStreamArg The message channel event which will be active pushed out during the callback
/// pUser  user defined data
...
/// \brief 事件订阅对象参数
/// \~english
/// \brief event subscribe info object parameter
...
/// \~english
/// \brief camera object interface class
/// \defgroup GENICAM_GigECamera  API for GigEVision Cameras Only
/// @{
...
/// \~english
/// \brief create GigE camera object's info object parameter
...
/// \defgroup GigEInterface API for GigEVision interface Only
/// @{

// 32位系统下128字节
/// \~chinese
/// \brief 创建GigECamera接口对象
/// \~english
...
typedef struct GENICAM_GigEInterface
{
	...
	/// \~english
	/// \param [in] thiz this pointer
	/// \brief network card description
	...
	/// \brief get Gateway
	/// \param [in] thiz this pointer
	/// \\brief return return NIC's Gateway, return NULL if failed
...
/// \~english
/// \brief create GigE camera interface object parameter
...
/// \param [out] ppGigEInterface interface object which connect the GigE Camera
/// \retval < 0 create failed
/// \retval 0   create successfully
...
/// \~english
/// \brief camera object interface class
/// \defgroup GENICAM_UsbCamera API for U3V Cameras Only
/// @{
...
typedef struct GENICAM_UsbCamera
{
	...
	/// \return 0 支持, -1 不支持
	/// \~english
	/// \brief Low Speed whether is supported or not
	/// \param [in] thiz this pointer
	...
	/// \return 0 支持, -1 不支持
	/// \~english
	/// \brief Full Speed whether is supported or not
	/// \param [in] thiz this pointer
	/// \return 0 if supported, return -1 if not supported
	...
	/// \return 0 支持, -1 不支持
	/// \~english
	/// \brief High Speed whether is supported or not
	/// \param [in] thiz this pointer
	/// \return 0 if supported, return -1 if not supported
	...
	/// \return 0 支持, -1 不支持
	/// \~english
	/// \brief Super Speed whether is supported or not
	/// \param [in] thiz this pointer
	/// \return 0 if supported, return -1 if not supported
...
/// \~english
/// \brief create Usb camera object's info object parameter
...
/// \~chinese
/// \brief 相机接口类
/// \defgroup GENICAM_UsbInterface  连接USB相机的接口
/// \~english
/// \brief camera interface 
/// \defgroup GENICAM_UsbInterface API for U3V interface Only
/// @{
...
typedef struct GENICAM_UsbInterface
{
	...
	/// \~english
	/// \param [in] thiz this pointer
	/// \brief return description of USB interface
	/// \return NULL if failed
	const char * (*getDescription)(struct GENICAM_UsbInterface *thiz);

	/// \~chinese
	/// \param [in] thiz 本接口指针
	/// \brief 返回 USB接口的Vendor ID
	/// \return 失败返回NULL
	/// \~english
	/// \param [in] thiz this pointer
	/// \brief return Vendor ID of USB interface
	/// \return NULL if failed
	const char * (*getVendorID)(struct GENICAM_UsbInterface *thiz);

	/// \~chinese
	/// \param [in] thiz 本接口指针
	/// \brief 返回 USB接口的Device ID，
	/// \return 失败返回NULL
	/// \~english
	/// \param [in] thiz this pointer
	/// \brief return Device ID of USB interface
	/// \return NULL if failed
	const char * (*getDeviceID)(struct GENICAM_UsbInterface *thiz);

	/// \~chinese
	/// \param [in] thiz 本接口指针
	/// \brief 返回 USB接口的Subsystem ID，
	/// \return 失败返回NULL
	/// \~english
	/// \param [in] thiz this pointer
	/// \brief return Subsystem ID of USB interface
	///  \return NULL if failed
	const char * (*getSubsystemID)(struct GENICAM_UsbInterface *thiz);

	/// \~chinese
	/// \param [in] thiz 本接口指针
	/// \brief 返回 USB接口的evision，
	///  \return 失败返回NULL
	/// \~english
	/// \param [in] thiz this pointer
	/// \brief return Revision of USB interface 
	///  \return NULL if failed
	...
	/// \~english
	/// \param [in] thiz this pointer
	/// \brief return speed of USB interface
	/// return NULL if failed
...
/// \brief 创建USB设备接口对象参数
/// \~english
/// \brief create USB camera interface object parameter
...
/// \param [out] ppUsbInterface interface object which connect the USB Camera
/// \retval < 0 create failed
/// \retval 0   create successfully
...
/// \retval 0   创建成功
/// \~english
/// \brief create double node object
...
/// \retval 0   创建成功
/// \~english
/// \brief create enum node type object
...
/// \retval 0   创建成功
/// \~english
/// \brief create command node object
...
/// \retval 0   创建成功
/// \~english
/// \brief create string node object
...
typedef struct GENICAM_AcquisitionControl
{
	...
	/// \~chinese
	/// \brief 获取acquisitionMode属性操作对象
	/// \param [in] thiz 本接口指针
	/// \return 返回属性操作对象,失败返回NULL
	/// \~english
	/// \brief get acquisitionMode property operation object
	/// \param [in] thiz this pointer
	/// \return success:return property operation object, fail:return NULL
...
typedef struct GENICAM_AnalogControl
{
	...
	/// \~chinese
	/// \brief 获取BlackLevelSelector属性操作对象，选择操作哪个黑电平
	/// \param [in] thiz 本接口指针
	/// \return 返回属性操作对象，失败返回NULL
	/// \~english
	/// \brief get BlackLevelSelector property operation object, select which black level is controlled
	/// \param [in] thiz this pointer
	/// \return success:return property operation object, fail:return NULL
...
/// \~english
/// \brief DeviceControl property operation class
typedef struct GENICAM_DeviceControl
{
	...
	/// \return 返回属性操作对象，失败返回NULL
	/// \~english
	/// \brief get DeviceUserID property operation object which is readable and writeable
	/// \param [in] thiz this pointer
	/// \return success:return property operation object, fail:return NULL
...
/// \~english
/// \brief DigitalIOControl property operation class
typedef struct GENICAM_DigitalIOControl
{
	...
	/// \return 返回属性操作对象，失败返回NULL
	/// \~english
	/// \brief get LineSelector property operation object which is readable and writeable
	/// \param [in] thiz this pointer
	/// \return success:return property operation object, fail:return NULL
	...
	/// \return 返回属性操作对象，失败返回NULL
	/// \~english
	/// \brief get LineDebouncerTimeAbs property operation object which is readable and writeable 
	/// \param [in] thiz this pointer
	/// \return success:return property operation object, fail:return NULL
	...
	/// \return 返回属性操作对象，失败返回NULL
	/// \~english
	/// \brief get UserOutputSelector property operation object which is readable and writeable
	/// \param [in] thiz this pointer
	/// \return success:return property operation object, fail:return NULL
	...
	/// \return 返回属性操作对象，失败返回NULL
	/// \~english
	/// \brief get UserOutputValue's property operation object which is readable and writeable
	/// \param [in] thiz this pointer
	/// \return success:return property operation object, fail:return NULL
