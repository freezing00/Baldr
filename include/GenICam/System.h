...
public:
	...
	/// \~chinese
	/// \brief 根据 DeviceUserID 获取 Camera 智能指针
	/// \param [in] pDeviceUserID 相机的DeviceUserID
	/// \return Camera 智能指针
	/// 若传入的pDeviceUserID不存在或者非法，则返回空智能指针对象.
	/// 若设备列表中存在重复DeviceUserID的相机，则返回列表中第一个对应的DeviceUserID的相机对象.
	/// \~english
	/// \brief get smart pointers to Camera according to Camera DeviceUserID
	/// \param [in] pDeviceUserID Camera's DeviceUserID
	/// \return Camera smart pointer. 
	/// if the parameter pDeviceUserID does not exist or is invalid, return empty smart pointer object
	/// if the DeviceUserID in the device list is duplicated, return the first camera pointer that matches the DeviceUserID.
	ICameraPtr getCameraPtrByDeviceUserID(const char * pDeviceUserID) const;

	/// \~chinese
	/// \brief 根据 IP地址 获取 Camera 智能指针，仅限Gige设备使用.
	/// \param [in] pIPAddress 相机的IP地址
	/// \return Camera 智能指针,若传入的pIPAddress不存在或者非法，则返回空智能指针对象.
	/// \~english
	/// \brief get smart pointers to Camera according to Camera IP address. Only for Gige device.
	/// \param [in] pIPAddress Camera's IP address
	/// \return Camera smart pointer. If the parameter pIpAddress does not exist or is invalid, return empty smart pointer object
	ICameraPtr getCameraPtrByIP(const char * pIPAddress) const;
