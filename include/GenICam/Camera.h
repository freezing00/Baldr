...
public:
	...
	/// \param [out] vErrorList  加载失败的属性名列表。一维表示属性个数，最大值为MAX_PARAM_CNT。
    /// \retval 0       成功
    /// \retval 非零    失败
    /// \~english 
	...
	/// \param [out] vErrorList the list of load failed properties. maximum length of chars is MAX_STRING_LENTH.
    /// \retval 0 success
    /// \retval non-zero fail
